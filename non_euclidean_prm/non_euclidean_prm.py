...
N_KNN = 10 # number of neighbors to attempt to connect
MAX_EDGE_LEN = 0.01  # [m] Maximum edge length
...
def sample_points(arm, obstacles, sx, sy, gx, gy):
    ...
    while len(x) != N_SAMPLE + 1 and len(y) != N_SAMPLE + 1:
      # should lie in -pi =< r =< pi
      coordinates = [math.radians(np.random.randint(-180, 181)),
                     math.radians(np.random.randint(-180, 181))]
      if not detect_collision(arm, obstacles, coordinates):
...
def construct_prm(arm, obstacles, sample_x, sample_y):
    ...
    for i in range(len(sample_x)):
      points.append([sample_x[i], sample_y[i]])

    # I used MAX_EDGE_LEN for Radius variable of NearestNeighbors. 
    neighbors = NearestNeighbors(n_neighbors=N_KNN, radius=MAX_EDGE_LEN,
                                 metric=torus_dist)
    ...
    for i in range(len(indices)):
      ...
      for j in range(len(indices[i])):
        _, eu = toroid_to_euclidean_points(points[i], points[indices[i][j]])

        # Incremental Collision Detection
        n = 10
        x_s = (eu[0] - points[i][0]) / n
        y_s = (eu[1] - points[i][1]) / n
        ...
        for k in range(n):
          x += step_size[0]
          y += step_size[1]
          if detect_collision(arm, obstacles, [x, y]):
            collided = True

        if not collided:
          road.append(indices[i][j])
      roadmap.append(road)
